...
amts::Color amts::Renderer::per_pixel(const u64& x, const u64& y, const u64& width, const u64& height, const Scene& scene, const Camera& camera, const MaterialPool& materialPool) const {
...
void amts::Renderer::render(const std::unique_ptr<RenderingTarget>& target, const std::unique_ptr<Scene>& scene, const std::unique_ptr<Camera>& camera, const std::unique_ptr<MaterialPool>& materialPool) {
